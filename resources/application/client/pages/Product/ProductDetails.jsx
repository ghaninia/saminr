...
export default function ProductDetails() {
  ...
  const normalizedProduct = useMemo(() => {
    ...
    const image = typeof product?.image === 'string' && product.image.trim() ? product.image : DEFAULT_PRODUCT_IMAGE
    const gallery = Array.isArray(product?.gallery) ? product.gallery.filter((item) => typeof item === 'string' && item.trim()) : []
    const introVideo = typeof product?.intro_video === 'string' && product.intro_video.trim() ? product.intro_video : ''
    const variants = Array.isArray(product?.variants) ? product.variants : []
    const colors = Array.isArray(product?.colors) ? product.colors : []
    const summaryAttributes = Array.isArray(product?.summary_attributes) ? product.summary_attributes : []
    ...
    const mediaItems = [
      ...
      ...gallery.map((url, index) => ({ type: 'image', url, label: `${title || t('productDetails.mediaProductFallback')} ${index + 1}` })),
      ...(introVideo ? [{ type: 'video', url: introVideo, label: `${title || t('productDetails.mediaProductFallback')} ${t('productDetails.mediaVideoSuffix')}` }] : []),
    ]
  ...
  const activeVariant = useMemo(() => {
    ...
    return visibleVariants.find((variant) => variant?.is_default) ?? visibleVariants[0] ?? normalizedProduct.defaultVariant ?? null
  }, [normalizedProduct, visibleVariants])

  if (loading) {
    return <section className="mx-auto max-w-7xl px-4 py-16 text-sm text-slate-500">{t('productDetails.loading')}</section>
  }
  ...
  const summarySpecs = normalizedProduct.summaryAttributes.map((attribute, index) => {
    const label = resolveLocalizedValue(attribute?.label, attribute?.label_i18n, language)
    ...
    const fallbackDefaultValue = resolveLocalizedValue(attribute?.default_value, attribute?.default_value_i18n, language)
  ...
  const variantSpecs = Array.isArray(activeVariant?.attributes)
    ? activeVariant.attributes
        ...
        .map((attribute, index) => ({
          key: String(attribute?.key ?? index),
          label: resolveLocalizedValue(attribute?.label ?? attribute?.key, attribute?.label_i18n, language),
  ...
  return (
    <>
    <section
      className="banner-header section-padding bg-img" data-overlay-dark="5"
      data-overlay-dark="5"
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {normalizedProduct.colors.map((colorItem) => {
                        ...
                        const label = resolveLocalizedValue(colorItem.name, colorItem.name_i18n, language)
                        ...
                        return (
                          <button
                            ...
                            className={`h-8 w-8 rounded-full border-2 transition ${isSelected ? 'border-[#f5b754] ring-2 ring-[#f5b754]/30' : 'border-white/30'}`}
                            title={label}
                            aria-label={label}
                  ...
                  {renderedSpecs.length > 0 ? (
                    <div className="space-y-3">
                      {renderedSpecs.map((item) => (
                        <div key={item.key} className="grid grid-cols-2 gap-3 border-b border-white/10 pb-3 text-sm last:border-b-0 last:pb-0">
